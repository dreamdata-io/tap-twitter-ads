...
def get_async_data(report_name, client, url):
    ...
    try:
        response = Request(
            client, "get", resource.path, domain=domain, raw_body=True, stream=True
        ).perform()
...
def sync_endpoint(
    ...
    for sub_type in sub_types:
            ...
            for record in cursor:
                ...
                if bookmark_field:
                    ...
                    if bookmark_value_str:
                        ...
                        # pylint: disable=line-too-long
                        LOGGER.info(
                            "Stream: {} - NO BOOKMARK, bookmark_field: {}, record: {}".format(
                    ...
                    if bookmark_value < last_dttm:
                        # Finish looping
                        LOGGER.info(
                            "Stream: {} - Finished, bookmark value < last datetime".format(
                                stream_name
                            )
                        )
                        break
        ...
        if children:
            for child_stream_name, child_endpoint_config in children.items():
                if child_stream_name in child_streams:
                    ...
                    # pylint: disable=line-too-long
                    LOGGER.info(
                        "Child Stream: {} - START Syncing, parent_stream: {}, account_id: {}".format(
                    ...
                    # pylint: disable=line-too-long
                    LOGGER.info(
                        "Child Stream: {} - FINISHED Syncing, parent_stream: {}, account_id: {}".format(
                            child_stream_name, stream_name, account_id
                        )
                    )
                    # pylint: enable=line-too-long
                    LOGGER.info(
                        "Child Stream: {} - total_records: {}".format(
        ...
        # pylint: disable=line-too-long
        LOGGER.info(
            "Stream: {}, Account ID: {} - FINISHED Sub Type: {}, Total Sub Type Records: {}".format(
...
def get_absolute_start_end_time(
...
def get_active_entity_sets(
    ...
    for placement in PLACEMENTS:  # ALL_ON_TWITTER, PUBLISHER_NETWORK
        ...
        for active_entity in active_entities:
            ...
            LOGGER.info(
                "active_entity_dict = {}".format(active_entity_dict)
            )  # COMMENT OUT
            entity_id = active_entity_dict.get("entity_id")
            entity_placements = active_entity_dict.get("placements", [])
            entity_start = strptime_to_utc(
                active_entity_dict.get("activity_start_time")
            ).astimezone(timezone)
            entity_end = strptime_to_utc(
                active_entity_dict.get("activity_end_time")
            ).astimezone(timezone)
...
def post_queued_async_jobs(
    ...
    for chunk_ids in split_list(entity_ids, 20):
        ...
        LOGGER.info(
            "Report: {} - POST ASYNC queued_job, chunk#: {}".format(report_name, chunk)
        )
        ...
        LOGGER.info(
            "Report: {} - queued_job POST URL: {}/{}/{}".format(
                report_name, ADS_API_URL, API_VERSION, queued_job_path
            )
        )
        LOGGER.info(
            "Report: {} - queued_job params: {}".format(report_name, queued_job_params)
        )

        # POST queued_job: asynchronous job
        queued_job = post_resource(
            "queued_job", client, queued_job_path, queued_job_params
        )
...
def get_async_results_urls(client, account_id, report_name, queued_job_ids):
    ...
    while len(queued_job_ids) > 0 and jobs_still_running and j <= 20:
        ...
        wait_sec = 15
        LOGGER.info(
            "Report: {} - Waiting {} sec for async job to finish".format(
        ...
        LOGGER.info(
            "Report: {} - async_job_statuses GET URL: {}/{}/{}".format(
                report_name, ADS_API_URL, API_VERSION, async_job_statuses_path
            )
        )
        LOGGER.info(
            "Report: {} - async_job_statuses params: {}".format(
                report_name, async_job_statuses_params
            )
        )
        async_job_statuses = get_resource(
            "async_job_statuses",
            client,
        ...
        for async_job_status in async_job_statuses:
            ...
            if job_status == "PROCESSING":
                ...
                LOGGER.info(
                    "Report: {} - job_id: {}, finished running (SUCCESS)".format(
                        report_name, job_id
                    )
                )
...
def sync_report(
    ...
    LOGGER.info(
        "Report: {}, Entity: {}, Segment: {}, Granularity: {}".format(
    ...
    while window_start != abs_end:
        ...
        # ORGANIC_TWEET cannot use active_entities endpoint
            ...
            LOGGER.info(
                "Report: {} - GET ORGANINC_TWEET entity_ids".format(report_name)
            )
            entity_ids = get_tweet_entity_ids(
                client, account_id, window_start, window_end
            )
        ...
        # ALL OTHER entity types use active_entities endpoint
            ...
            LOGGER.info(
                "Report: {} - GET {} active_entities entity_ids".format(
            ...
            LOGGER.info(
                "Report: {} - active_entities GET URL: {}/{}/{}".format(
                    report_name, ADS_API_URL, API_VERSION, active_entities_path
                )
            )
            LOGGER.info(
                "Report: {} - active_entities params: {}".format(
                    report_name, active_entities_params
                )
            )
            active_entities = get_resource(
                "active_entities", client, active_entities_path, active_entities_params
            )

            # Get active entity_ids, start, end for each placement type for date window
            window_entity_id_sets = []
            window_entity_id_sets = get_active_entity_sets(
                active_entities,
                report_name,
                account_id,
                report_entity,
    ...
    for sub_type_id in sub_type_ids:
        ...
        for entity_id_set in entity_id_sets:
            ...
            sub_type_queued_job_ids = (
                sub_type_queued_job_ids + entity_id_set_queued_job_ids
            )
    ...
    # GET ASYNC Status Reference: https://developer.twitter.com/en/docs/ads/analytics/api-reference/asynchronous#get-stats-jobs-accounts-account-id
    async_results_urls = []
    async_results_urls = get_async_results_urls(
        client, account_id, report_name, queued_job_ids
    )
    ...
    for async_results_url in async_results_urls:
        ...
        LOGGER.info(
            "Report: {} - GET async data from URL: {}".format(
        ...
        if transformed_data is None or transformed_data == []:
            LOGGER.info(
                "Report: {} - NO TRANSFORMED DATA for URL: {}".format(
                    report_name, async_results_url
                )
            )
...
def sync(client, config, state):
    ...
    for account_id in account_list:
        ...
        for stream_name in parent_streams:
            ...
            LOGGER.info(
                "Stream: {} - START Syncing, Account ID: {}".format(
            ...
            LOGGER.info(
                "Stream: {} - FINISHED Syncing, Account ID: {}, Total Records: {}".format(
        ...
        if report_streams != []:
            ...
            for country_code in country_code_list:
                ...
                country_cursor = get_resource(
                    "countries", client, country_path, country_params
                )
                for country in country_cursor:
                    country_id = country["targeting_value"]
                    country_ids.append(country_id)
            LOGGER.info(
                "Countries - Country Codes: {}, Country Targeting IDs: {}".format(
            ...
            platforms_cursor = get_resource(
                "platforms", client, platforms_path, platforms_params
            )
